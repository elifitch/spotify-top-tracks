...
function generateMockCSVs() {
    ...
    return Promise.all(locales.map(locale => {
        ...
        return request(requestUrl).then(data => {
            return {
                id: locale.id,
                chart: data
            }
        ...
        const allChartData = allData.reduce((allChartData, localeData) => {
            if (localeData) {
        ...
        fs.writeFileSync(`${__dirname}/mock-json/mock-csvs.json`, JSON.stringify(allChartData));
    });
}
